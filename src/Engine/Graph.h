...
typename Container::iterator push(Container& c, T v) {
    c.push_back(v);
    return prior(c.end());
}
...
public:
     ...
     */
     Graph_base(bool is_digraph = false) : m_is_digraph(is_digraph) { };
     template <typename EdgeIterator>
     Graph_base(EdgeIterator first, EdgeIterator last,
                node_size_t size, bool is_digraph = false) : m_nodes(size), m_is_digraph(is_digraph) {
         while (first != last) {
    ...
    edge_descriptor add_edge(node_descriptor v, node_descriptor u, _EdgeProperty e) {
        ...
        auto it = push(out_edge_list(v), stored_edge(u, e));

        if (m_is_digraph) {
            push(out_edge_list(u), stored_edge(v, e));
        }
    ...
    void remove_edge(edge_descriptor e) {
        ...
        if(m_is_digraph) {
            remove_directed_edge(out_edge_list(e.target()), *e._property);
        }
    }
    ...
    inline edge_list& out_edge_list(const node_descriptor v) {
     ...
     */
    inline const edge_list& out_edge_list(const node_descriptor v) const {
        return m_nodes[v].m_out_edges;
    }
